...
```
Note: Check if `Tamper Protection` is turned on. If so the previous command will not work and you have to disable `Real-Time protection` manually via the GUI!
