...
<div markdown="span" style="margin-bottom: 10px; margin-top: 10px; overflow: hidden; color: #31708f; background-color: #d9edf7; border-color: #bce8f1; padding: 15px; border: 1px solid transparent; border-radius: 4px;"><b>Note:</b> Check if `Tamper Protection` is turned on. If so the previous command will not work and you have to disable `Real-Time protection` manually via the GUI!</div>
...
There are several ways on how you can develop and test your shellcode on windows. One way is to make use python and the keystone-engine to compile our assembly instructions. However, I wanted to approach
 ...
 This way you can easily develop your windows shellcode using the Powershell ISE and do not have to install more software on your developing machine. Howeverm beforehand you have to set the execution-policy of powershell to `Bypass` by running
```powershell
Set-ExecutionPolicy Bypass
```
as an administrator. Afterwards you can use my [run-assembly.ps1](https://github.com/Shellpecker/PS-Shellcode-Runner/blob/main/run-assembly.ps1), which by default assembles a x64-shellcode by [g3tsyst3m](https://g3tsyst3m.github.io/shellcoding/assembly/debugging/x64-Assembly-and-Shellcoding-101-Part-4/) that runs `calc.exe` by calling `WinExec`
```powershell
# Architecture
$architecture = 64
#$architecture = 32
